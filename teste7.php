...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['contratos'])) {
    ...
    ?>
        <form method='POST' action=''> Já tem contrato selecionado e profissional selecionado
        <div class="form-group">
            <label>Contrato(s) selecionado(s): <a href='#' id='reset-button' onclick='limparCampos()'><img src='assets/img/lock.jpeg' width='30px'></a></label>
            <div class="d-flex">            
                        ...
                        if ($resultado_profissionais->num_rows > 0) {
                            while ($dados_profissional = $resultado_profissionais->fetch_assoc()) {
                                ...
                                if (!in_array($id_profissional, $profissionaisAdicionados)) {
                                    ...
                                    $checked_profissionais = in_array($id_profissional, $profissionaisSelecionados) ? 'checked' : '';
                                    echo '<label class="selectgroup-item">
                                            <input type="checkbox" name="profissionais[]" onclick="return false;"
                                        ' . $checked_profissionais . ' value="' . $id_profissional . '" class="selectgroup-input profissionais" />
