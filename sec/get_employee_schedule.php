...
$sql = "SELECT DISTINCT e.id, e.id_usuario, e.id_afastamento, e.horarioinicio, e.horariofim, e.local, e.dia, e.mes, e.ano, r.id_usuario, r.id_cliente
FROM escala e
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        $schedule = $motivo_afastamento ? 'afastado' : $row['horarioinicio'] . ' - ' . $row['horariofim'];
        $location = $motivo_afastamento ? '' : $row['local'];
